...
% -------------------------------------------------------------------------
% STATEMENT:
% This file is part of PET Recon Package by Saeed Ashrafinia, www.Qurit.ca
% Code has been slighty modified by Kyung-Nam Lee in consultation with Arman Rahmim for ease of use (e.g. clarity on input parameters, dimensions, etc.); 2022 (if you have further comments or
% suggestions please reach out to us)
% --> Copyright (C) 2013-  Saeed Ashrafinia, Arman Rahmim (Johns Hopkins
...
%% Defining Variables
PSF_Kernel  = 0;        % 0 for no PSF modeling, 1 for true PSF modeling, 2 for no PSF + true PSF modeling, 3 for generalized PSF modeling (no PSF + true PSF + under- and over-estimated PSFs). Generalized PSF are defined in Gen_PSF_Kernels.m.

Num_Noise_Realz = 5;                  % The # of Noise Realizations (same as # of realistic Recon Images)
NOISE_REALZ_Mean_Recon_Img = 1;       % Number of noise realizations for Calc. of Mean of Recon. Images (i.e., these are averaged to create an overall recon image) 
...
ydim        =xdim;      % Assuming that xdim==ydim.
input_xdim  = xdim;      % Size of the input image (assuming that size of recon. image is same as the size of entered true image)
IMG_ABS_PRS = 0;        % 0: run for signal absent; 1: present; 2: absent and present
...
HIGH_RES_TRUE = 0;      % Use a higher resolution image as the true input and then scale down it projection
LOAD_ATTENUATION = 1;   % Load attenuation map or consider a matrix of zeros
...
% Unit: 1) Activity: [Bq], [kBq], [MBq] , 2) Concentration: [Bq/ml], [kBq/ml], [MBq/ml]
AOC_ind=1; % 1: Activity (e.g. Bq), 2: Concentration (e.g. Bq/ml)
AOC_unit=2; % 1: [Bq] or [Bq/ml], 2: [kBq] or [kBq/ml], 3) [MBq] or [MBq/ml]
load ./input/Input_Activity_kBq.mat; 
if AOC_ind==1 % Activity
    image_true=Input_Activity;
    CORRECT_IMG_UPTKE = 1; % 1: Converting into Concentration , 2: X Converting
else % Concentration
    image_true=Input_Concent;
    CORRECT_IMG_UPTKE = 2;
end
% Loading Attenuation Map %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if LOAD_ATTENUATION
    load ./input/Input_Atten_511keV.mat % Unit must be: [Linear Attenuation Coeff./voxel]
    mu_map=Input_Atten;
else
...
TmrCount        = 1;    % Select from 1 to 8 different tumors, Refer to Gen_Image_True for indices. This variable is only used for naming the output image.   

%% Continue Initializing
ScanDuration    = 3;   % Scan duration in minutes
...
voxel_size      = 2; % Voxel size [mm] for Axial Slice (Transvverse Direction)

%TotalCounts     = 1e9; % This vaariable is removed b/c it does not make any effect on calibration factor. 
...
if CORRECT_IMG_UPTKE==1           
    image_correction = 1 ./ ( voxel_size/10 * voxel_size/10 * d_z/10); % to convert [Bq] ==> [Bq/ml], [kBq] ==> [kBq/ml], or [MBq] ==> [MBq/ml]
else
    image_correction = 1;  % Nothing to do (b/c Concentration data is already entered)
end
...
%         image_true = image_true .* C; %                                             Mask with giant circle
    ...
    image_true = image_true * image_correction; % Concentration Image [Bq/ml], [kBq/ml], or [MBq/ml]
...
%         atten(:,:,m)  = exp(-radon(mu_map , theta_m(:,m))*bin_size);
        atten(:,:,m)  = exp(-radon(mu_map , theta_m(:,m)));
    end
...
%     disp(' ');
    ...
    calib_fact = calib_factor(xdim , ydim , VCT_sensitivity , theta_m , norm_image , NUM_BINS) ;
    scale_factor = scaling_factor(AOC_unit , voxel_size/HiResScale/10 , voxel_size/HiResScale/10 , d_z / 10 , ...
        start_time , end___time , lambda , calib_fact , IMAGE_DECAYED);
    
    %% Create Empty Matrices to save reconstructed images
    if      img_prs_abs == 1
        SIG_ABS         = single(zeros(numel(BkgndBox(1):BkgndBox(2)) , numel(BkgndBox(3):BkgndBox(4)) , ITERATIONS , NOISE_REALZ_Mean_Recon_Img , NUMVAR));
        SIG_ABS_NF      = single(zeros(numel(BkgndBox(1):BkgndBox(2)) , numel(BkgndBox(3):BkgndBox(4)) , ITERATIONS , NUMVAR));
    elseif  img_prs_abs == 2
        SIG_PRS         = single(zeros(numel(BkgndBox(1):BkgndBox(2)) , numel(BkgndBox(3):BkgndBox(4)) , ITERATIONS , NOISE_REALZ_Mean_Recon_Img , NUMVAR));
    ...
    Y_bar_realization = zeros(binsdim,thdim_m,SUBSETS,NOISE_REALZ_Mean_Recon_Img);
...
%                         end
        ...
        elseif IsNoisy == 1
            if img_prs_abs == 2
                Mean_SIG_PRS = MeanImg / NOISE_REALZ_Mean_Recon_Img;
                FileName = ['SIG_PRS_NF__',int2str(xdim),'_R',int2str(NOISE_REALZ_Mean_Recon_Img),'_I',int2str(ITERATIONS),'_S',int2str(SUBSETS),'_K',int2str(NUMVAR),'_Tmr',int2str(TmrCount),'.mat'];
                Path_str = [DIRECTORY,FileName];
                %save (Path_str ,'SIG_PRS_NF' , 'image_true' , '-v7.3');
            elseif img_prs_abs == 1
                Mean_SIG_ABS = MeanImg / NOISE_REALZ_Mean_Recon_Img;
                FileName = ['SIG_ABS_NF__',int2str(xdim),'_R',int2str(NOISE_REALZ_Mean_Recon_Img),'_I',int2str(ITERATIONS),'_S',int2str(SUBSETS),'_K',int2str(NUMVAR),'_Tmr',int2str(TmrCount),'.mat'];
                Path_str = [DIRECTORY,FileName];
        ...
        Recon_Img{Realz,1}=MeanImg(:,:,ITERATIONS) ./ NOISE_REALZ_Mean_Recon_Img; % Reconstucted Image with Noise! 
end

%% Saving Reconstructed Image into Mat.file
fname= strcat('./output/Recon_Img_Iter',int2str(ITERATIONS),'_Subset_',int2str(SUBSETS),'.mat');
save(fname, 'Recon_Img');
...
%        FileName = ['SIG_PRS_ALL__',int2str(xdim),'_R',int2str(NOISE_REALZ_Mean_Recon_Img),'_I',int2str(ITERATIONS),'_S',int2str(SUBSETS),'_K',int2str(NUMVAR),'_Tmr',int2str(TmrCount),'.mat'];
%        Path_str = [DIRECTORY,FileName];
